...
public abstract class UIBasicSprite : UIWidget
{
	public enum Type
	{
		...
		Advanced,
        Mirror, ///镜像类型;
        MirrorFilled,///镜像+比例切割（暂时只支持横向切割）;
        SlicedFilled,///局部拉伸a+比例切割（暂时只支持横向与纵向切割）;
        MirrorVertically,   ///纵向镜像;
        MirrorHorizonVertically,//水平加纵向镜像
        MirrorHorizonSliced,//垂直镜像加拉伸
    }
...
#region Fill Functions
	...
	protected void Fill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols, Rect outer, Rect inner)
	{
		...
		switch (type)
		{
			...
			break;
            case Type.Mirror:
                MirrorFill(verts, uvs, cols);
                break;

            case Type.MirrorVertically:
                MirrorVerticallyFill(verts, uvs, cols);
                break;
            case Type.MirrorFilled:
                MirrorFilledFill(verts, uvs, cols);
                break;

            case Type.SlicedFilled:
                SlicedFilledFill(verts, uvs, cols);
                break;
            case Type.MirrorHorizonVertically:
                MirrorHorizonVerticallyFill(verts, uvs, cols);
                break;
            case Type.MirrorHorizonSliced:
                MirrorHorizonSlicedFill(verts, uvs, cols);
                break;
        }
	}
	...
	void FilledFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
	{
		...
		Vector4 v = drawingDimensions;
		Vector4 u = drawingUVs;
		Color32 c = drawingColor.GammaToLinearSpace();
    ...
    void MirrorHorizonSlicedFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
    {

        Vector4 v = drawingDimensions;
        //Vector4 u = drawingUVs;
        Color32 c = drawingColor;
        float midX = (v.z + v.x) / 2.0f;
    ...
    void MirrorHorizonVerticallyFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
    {
        Vector4 v = drawingDimensions;
        Vector4 u = drawingUVs;
        Color32 c = drawingColor;
        float midX = (v.z + v.x) / 2.0f;
        float midY = (v.w + v.y) / 2.0f;
        //左上
        verts.Add(new Vector3(v.x, midY));
        verts.Add(new Vector3(v.x, v.w));
        verts.Add(new Vector3(midX, v.w));
        verts.Add(new Vector3(midX, midY));

        verts.Add(new Vector3(midX, midY));
        verts.Add(new Vector3(midX, v.w));
        verts.Add(new Vector3(v.z, v.w));
        verts.Add(new Vector3(v.z, midY));
        ...
        verts.Add(new Vector3(v.x, midY));
        verts.Add(new Vector3(midX, midY));
        verts.Add(new Vector3(midX, v.y));
        //右下
        verts.Add(new Vector3(midX, v.y));
        verts.Add(new Vector3(midX, midY));
        verts.Add(new Vector3(v.z, midY));
    ...
    /// 镜像贴图顶点构造;
    /// </summary>
    void MirrorFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
    {
        Vector4 v = drawingDimensions;
        Vector4 u = drawingUVs;
        Color32 c = drawingColor;
        ...
        verts.Add(new Vector3(v.x, v.y));
        verts.Add(new Vector3(v.x, v.w));
        verts.Add(new Vector3(midX/* + pixelSize*/, v.w));
        verts.Add(new Vector3(midX/* + pixelSize*/, v.y));
    ...
    void MirrorVerticallyFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
    {
        Vector4 v = drawingDimensions;
        Vector4 u = drawingUVs;
        Color32 c = drawingColor;
    ...
    void SlicedFilledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
    {
        Vector4 br = border * pixelSize;
        ...
        if ((mFillDirection != FillDirection.Horizontal) && (mFillDirection != FillDirection.Vertical) || (mFillAmount == 1.0f)) ///只支持横向裁切和竖向裁切;
        {
            SlicedFill(verts, uvs, cols);
            return;
        }
        if (mFillAmount <= 0.0f)        ///填充率为0时不画;
        {
            return;
        }
        ...
        //GTDebug.LogError("SlicedFilledFill drawingDimensions=" + v);
        ...
        float[] clipSide = new float[] { 0.0f, 0.0f };
        float length = 0.0f;
        //float uvLenth = 0.0f;
        if (mFillDirection == FillDirection.Horizontal)
        {
            length = (mTempPos[3].x - mTempPos[0].x);
            clipSide[0] = (mTempPos[1].x - mTempPos[0].x) / length;
            clipSide[1] = (mTempPos[2].x - mTempPos[0].x) / length;

            //uvLenth = (mTempUVs[3].x - mTempUVs[0].x);
        }
        else if (mFillDirection == FillDirection.Vertical)
        {
            length = (mTempPos[3].y - mTempPos[0].y);
            clipSide[0] = (mTempPos[1].y - mTempPos[0].y) / length;
            clipSide[1] = (mTempPos[2].y - mTempPos[0].y) / length;

            //uvLenth = (mTempUVs[3].y - mTempUVs[0].y);
        }

        float clipPer = 1.0f - mFillAmount;
        for (int x = 0; x < 3; ++x)
        {
            int x2 = x + 1;
            ...
            for (int y = 0; y < 3; ++y)
            {
                ...
                bool needTranform = false;

                if (mFillDirection == FillDirection.Horizontal)
                {
                    if (mInvert == true)
                    {
                        if ((x2 < 3) && (clipPer >= clipSide[x2 - 1]))
                        {
                            continue;
                        }
                        if ((x == 0) || (clipPer > clipSide[x - 1]))
                        {
                            needTranform = true;
                        }
                    }
                    ...
                    {
                        if (x > 0 && mFillAmount <= clipSide[x - 1])
                        {
                            continue;
                        }
                        if ((x2 == 3) || (mFillAmount < clipSide[x2 - 1]))
                        {
                            needTranform = true;
                        }
                    }
                ...
                else
                {
                    if (mInvert == true)
                    {
                        if ((y2 < 3) && (clipPer >= clipSide[y2 - 1]))
                        {
                            continue;
                        }
                        if ((y == 0) || (clipPer > clipSide[y - 1]))
                        {
                            needTranform = true;
                        }
                    }
                    ...
                    {
                        if (y > 0 && mFillAmount <= clipSide[y - 1])
                        {
                            continue;
                        }
                        if ((y2 == 3) || (mFillAmount < clipSide[y2 - 1]))
                        {
                            needTranform = true;
                        }
                    }
                ...
                //if (centerType == AdvancedType.Invisible && x == 1 && y == 1) continue;
                Vector3[] inV = new Vector3[]{new Vector3(mTempPos[x].x, mTempPos[y].y),
                    new Vector3(mTempPos[x].x, mTempPos[y2].y),
                    new Vector3(mTempPos[x2].x, mTempPos[y2].y),
                    new Vector3(mTempPos[x2].x, mTempPos[y].y)};


                Vector2[] inU = new Vector2[] { new Vector2(mTempUVs[x].x, mTempUVs[y].y),
                    new Vector2(mTempUVs[x].x, mTempUVs[y2].y),
                    new Vector2(mTempUVs[x2].x, mTempUVs[y2].y),
                    new Vector2(mTempUVs[x2].x, mTempUVs[y].y) };

                if (needTranform == true)       ///变换顶点和UV;
                {
                    bool tranUv = true;
                    if ((x == 1) && (y == 1))
                    {
                    ...
                    if (mFillDirection == FillDirection.Horizontal)
                    {
                        if (mInvert == true)
                        {
                            inV[0].x = inV[1].x = mTempPos[0].x + clipPer * length;

                            if (tranUv == true)
                            {
                                float per = (inV[0].x - mTempPos[x].x) / (mTempPos[x2].x - mTempPos[x].x);
                                inU[0].x = inU[1].x = mTempUVs[x].x + per * (mTempUVs[x2].x - mTempUVs[x].x);
                            }
                        }
                        else
                        {
                            inV[2].x = inV[3].x = mTempPos[0].x + mFillAmount * length;
                            if (tranUv == true)
                            {
                                float per = (inV[2].x - mTempPos[x].x) / (mTempPos[x2].x - mTempPos[x].x);
                                inU[2].x = inU[3].x = mTempUVs[x].x + per * (mTempUVs[x2].x - mTempUVs[x].x);
                            }
                        }
                    }
                    else
                    {
                        if (mInvert == true)
                        {
                            inV[0].y = inV[3].y = mTempPos[0].y + clipPer * length;
                            if (tranUv == true)
                            {
                                //inU[0].y = inU[3].y = mTempUVs[0].y + clipPer * uvLenth;
                                float per = (inV[0].y - mTempPos[y].y) / (mTempPos[y2].y - mTempPos[y].y);
                                inU[0].y = inU[3].y = mTempUVs[y].y + per * (mTempUVs[y2].y - mTempUVs[y].y);
                            }
                        }
                        else
                        {
                            inV[1].y = inV[2].y = mTempPos[0].y + mFillAmount * length;
                            if (tranUv == true)
                            {
                                //inU[1].y = inU[2].y = mTempUVs[0].y + mFillAmount * uvLenth;
                                float per = (inV[2].y - mTempPos[y].y) / (mTempPos[y2].y - mTempPos[y].y);
                                inU[1].y = inU[2].y = mTempUVs[y].y + per * (mTempUVs[y2].y - mTempUVs[y].y);
                            }
                        }
                    }
    ...
    void MirrorFilledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
    {
        if (mFillDirection != FillDirection.Horizontal)
        {
            MirrorFill(verts, uvs, cols);
        ...
        Vector4 u = drawingUVs;
        Color32 c = drawingColor;
        float midX = (v.x + v.z) / 2.0f;

        ///左边图的顶点和uv;
        Vector4 v1 = new Vector4(v.x, v.y, midX, v.w);
        Vector4 u1 = u;

        ///右边图的顶点和uv;
        Vector4 v2 = new Vector4(midX, v.y, v.z, v.w);
        Vector4 u2 = new Vector4(u.z, u.y, u.x, u.w);
        ...
        if (mInvert == true)
        {
            float fillV = (v.z - v.x) * (1.0f - mFillAmount);
            if (mFillAmount <= 0.5f)        ///左边图不显示;
            {
                v1.x = -1f;
                v2.x = v.x + fillV;
                u2.x = u.z - ((u.z - u.x) * (1.0f - mFillAmount * 2));
            }
            else
            {
                v1.x = v.x + fillV;
                ...
                u1.x = u.x + 2f * (u.z - u.x) * (1.0f - mFillAmount);
        ...
        {
            float fillV = (v.z - v.x) * mFillAmount;
            if (mFillAmount <= 0.5f)        ///右边图不显示;
            {
                v2.x = -1f;
                v1.z = v.x + fillV;
                u1.z = u.x + (u.z - u.x) * mFillAmount * 2f;
            }
            else
            {
                v2.z = v.x + fillV;
                ...
                u2.z = u.x + 2f * (u.z - u.x) * (1.0f - mFillAmount);
        ...
        if (v1.x != -1f)
        {
            ...
            verts.Add(new Vector3(v1.x, v1.w));
            verts.Add(new Vector3(v1.z, v1.w));
            verts.Add(new Vector3(v1.z, v1.y));

            uvs.Add(new Vector2(u1.x, u1.y));
            uvs.Add(new Vector2(u1.x, u1.w));
            uvs.Add(new Vector2(u1.z, u1.w));
            uvs.Add(new Vector2(u1.z, u1.y));
        ...
        if (v2.x != -1f)
        {
            verts.Add(new Vector3(v2.x, v2.y));
            verts.Add(new Vector3(v2.x, v2.w));
            verts.Add(new Vector3(v2.z, v2.w));
            verts.Add(new Vector3(v2.z, v2.y));

            uvs.Add(new Vector2(u2.x, u2.y));
            uvs.Add(new Vector2(u2.x, u2.w));
            uvs.Add(new Vector2(u2.z, u2.w));
            uvs.Add(new Vector2(u2.z, u2.y));
