...
{
    static readonly float min_window_w = 1024;
    static readonly float min_window_h = 720;
    static readonly float max_window_w = 1280;
    static readonly float max_window_h = 768;
    ...
    public Camera Cam
    {
        get
        {
            if (mCamera == null)
            {
                UICamera uiCamera = GetComponentInChildren<UICamera>();
                if (uiCamera != null)
                {
    ...
    static public float GetScreenScale()
    {
        ...
        {
            float sw = min_window_w / w;
            float sh = min_window_h / h;
            screen_scale = Mathf.Min(sw, sh);
        }
        else if (w > min_window_w || h > max_window_h)
        {
            float sw = max_window_w / w;
            float sh = max_window_h / h;
            screen_scale = Mathf.Max(sw, sh);
        }
        return screen_scale;
    }
	...
	public void UpdateScale (bool updateAnchors = true)
	{
        ...
        {
            Cam.orthographicSize = max_window_h * 0.5f;
        }
    }
