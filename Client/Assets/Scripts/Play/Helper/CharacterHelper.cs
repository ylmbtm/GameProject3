...
public class CharacterHelper
{
    ...
    public static bool CalcDamage(Character attacker, Character defender, XDamage dmg, bool ignoreDefense)
    {
        ...
        if (hurtValue > 0)
        {
            ...
            else
            {
                ...
                if (attacker.Type == EActorType.PARTNER && attacker.Host == CharacterManager.Main)
                {
                    if (crit)
                    {
    ...
    public static bool CalcBackMP(Character attacker, Character defender, string name, float percent, int fixValue)
    {
        ...
        if (defender.Type == EActorType.PARTNER && defender.Host == CharacterManager.Main)
        {
            GTEventCenter.FireEvent(GTEventID.TYPE_UD_PARTNER_HP);
        }
        return true;
    }
    ...
    public static void CalcAddAttr(Character carryer, EAttr attrID, EAttr refAttrID, float percent, int fixValue, bool onBegin)
    {
        ...
        int perAttrValue = carryer.BaseAttr.GetAttr(refAttrID);
        int newAttrValue = 0;
        if(onBegin)
        {
    ...
    public static void UpdateHP(Character cc)
    {
        ...
        if (cc.Type == EActorType.PARTNER && cc.Host == CharacterManager.Main)
        {
            GTEventCenter.FireEvent(GTEventID.TYPE_UD_PARTNER_HP);
        }
        if (cc.IsBoss())
        {
            GTEventCenter.FireEvent(GTEventID.TYPE_UP_BOSS_HP);
        }
    }
}
