...
public class UIRelics : GTWindow
{
    ...
    protected override void OnAwake()
    {
        ...
        relicsName = transform.Find("RelicsName").GetComponent<UILabel>();
        relicsDesc = transform.Find("RelicsDesc").GetComponent<UILabel>();

        cardTrans = transform.Find("View1/Cards");
        cardTemp  = transform.Find("View1/Card").gameObject;
        cardTemp.SetActive(false);
        cardDrag  = transform.Find("Drag").gameObject;
        btnReturn = transform.Find("Top/Btn_Return").gameObject;
        btnUpgrade = transform.Find("View2/Btn_Upgrade").gameObject;
        btnSkill = transform.Find("View2/Btn_Skill").gameObject;
        btnUnload = transform.Find("View2/Btn_Unload").gameObject;
        btnBattle = transform.Find("View2/Btn_Battle").gameObject;
    ...
    private void ShowRelicsObj()
    {
        ...
        mModel.transform.localPosition = db.StagePos;
        mModel.transform.localEulerAngles = db.StageEuler;
    ...
    {
        GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
        RelicsService.Instance.TryUpgradeRelics(centerID);
    }

    private void OnBattleClick(GameObject go)
    {
        GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
        RelicsService.Instance.TryBattleRelics(centerID);
    }

    private void OnUnloadClick(GameObject go)
    {
        GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
        RelicsService.Instance.TryUnloadRelics(centerID);
    }
    ...
    private void ShowCurrRelicsView2()
    {
        ...
        for (int i = 0; i < mRelicsCharges.Count; i++)
        {
            ...
            if (relics == null)
            {
                ...
                tab.chargeProgressNum.text = GTTools.Format("{0}/{1}", 0, db.LevelRequireExp[0]);
            }
            else
            {
                int curExp = 0;
                ...
                int maxExp = db.LevelRequireExp[relics.Level];
            ...
            {
                GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
                RelicsService.Instance.TryChargeRelics(targetID, index);
            };
        }
    }
    ...
    private void ShowPropertyView()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            float ratio1 = 1;
            float ratio2 = ratio1 * (db.AddProperty / 100f + 1);
            float num1 = 0;
            float num2 = 0;
            ...
            {
                ratio1 = Mathf.Pow(db.AddProperty / 100f + 1, relics.Level + 1);
                ratio2 = ratio1 * (db.AddProperty / 100f + 1);
    ...
    private void ShowButtonsState()
    {
        ...
        if (relics != null)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                if (curExp < db.LevelRequireExp[relics.Level])
                {
                    canUpgrade = false;
                }
            }
    ...
    {
        GTEventCenter.AddHandler<int, int>(GTEventID.TYPE_CHARGE_RELICS, OnRecvChargeRelics);
        GTEventCenter.AddHandler<int>(GTEventID.TYPE_UPGRADE_RELICS, OnRecvUpgradeRelics);
        GTEventCenter.AddHandler<int>(GTEventID.TYPE_BATTLE_RELICS, OnRecvBattleRelics);
        GTEventCenter.AddHandler<int>(GTEventID.TYPE_UNLOAD_RELICS, OnRecvUnloadRelics);
    }

    protected override void OnDelHandler()
    {
        GTEventCenter.DelHandler<int, int>(GTEventID.TYPE_CHARGE_RELICS, OnRecvChargeRelics);
        GTEventCenter.DelHandler<int>(GTEventID.TYPE_UPGRADE_RELICS, OnRecvUpgradeRelics);
        GTEventCenter.DelHandler<int>(GTEventID.TYPE_BATTLE_RELICS, OnRecvBattleRelics);
        GTEventCenter.DelHandler<int>(GTEventID.TYPE_UNLOAD_RELICS, OnRecvUnloadRelics);
    }
