...
BOOL CPayManager::LoadData(CppMySQL3DB& tDBConnection)
{
	...
	while (!QueryResult.eof())
	{
		...
		pPayDataObject->m_nProductID = QueryResult.getInt64Field("productid");
		pPayDataObject->m_nBuyID = QueryResult.getInt64Field("buyid");
...
BOOL CPayManager::OnMsgCreatePayRecord(NetPacket* pNetPacket)
{
	...
	if (m_setCreatedOrder.find(pReq->orderid()) != m_setCreatedOrder.end())
	{
		...
		Ack.set_orderid(pReq->orderid());
		ServiceBase::GetInstancePtr()->SendMsgProtoBuf(pNetPacket->m_dwConnID, MSG_CREATE_PAYMENT_ACK, 0, 0, Ack);
	...
	pReq->set_proxyid(pNetPacket->m_dwConnID);
