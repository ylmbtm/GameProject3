...
BOOL CGridManager::AddObjectToGrid( CMapObject* pMapObject, FLOAT x, FLOAT z )
{
...
BOOL CGridManager::RemoveObjectFromGrid( CMapObject* pMapObject, FLOAT x, FLOAT z )
{
	CGrid* pGrid = GetGridByPos(x, z);
	if(pGrid == NULL)
	{
...
BOOL CGridManager::IsObjectExist(CMapObject* pMapObject, INT32 nIndex)
{
	...
	CMapObject* pObject = pGrid->GetHeadObject();
	if (pObject == NULL)
	{
		return FALSE;
	}
